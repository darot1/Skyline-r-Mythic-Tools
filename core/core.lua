...
local function CreateIcon(parent, tag)
	...
	if strfind(tag, "spells") then
		...
		button.cdtext = button.cd:GetRegions()
		button.cdtext:SetFont(font, SMT_CDB["PlateAlerts"]["cdfsize"])
...
local function OnNamePlateCreated(namePlate)
	...
	namePlate.suf.icons:SetPoint("BOTTOM", namePlate.suf, "TOP", SMT_CDB["PlateAlerts"]["x"], SMT_CDB["PlateAlerts"]["y"])
...
T.EditPlateIcons = function(tag)
	...
	if tag == "cooldown_font_size" or tag == "all" then
		for k, frame in pairs(G.Plate_IconHolders) do
			for j, icon in pairs(frame.Aura_Icons) do
				if icon.cdtext ~= nil then
					icon.cdtext:SetFont(font, SMT_CDB["PlateAlerts"]["cdfsize"], "OUTLINE")
				end
			end
			for j, icon in pairs(frame.Spell_Icons) do
				if icon.cdtext ~= nil then
	...
	if tag == "x" or tag == "y" or tag == "all" then
		for k, frame in pairs(G.Plate_IconHolders) do
			frame:SetPoint("BOTTOM", frame:GetParent(), "TOP", SMT_CDB["PlateAlerts"]["x"], SMT_CDB["PlateAlerts"]["y"])
		end
	end
end
----------------------------------------------------------
